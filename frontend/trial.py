...
class App:
    ...
    def toggle_play_pause(self):
        ...
        if self.is_playing:
            ...
            # Currently paused, so resume playing
            if pygame.mixer.music.get_busy():
                pygame.mixer.music.unpause()
            else:
                # If the song has ended or wasn't playing, start it
    ...
    def update_progress(self):
        if self.is_playing and pygame.mixer.music.get_busy():
            ...
            # Update progress bar (assuming a typical song length of 4 minutes)
            # You might want to get the actual length of each song for better accuracy
            song_length = 240  # 4 minutes in seconds
            ...
            # Find the current time label by its parent hierarchy instead of direct nametowidget
            for widget in self.root.winfo_children():
                if isinstance(widget, tk.Frame):
                    for child in widget.winfo_children():
                        if isinstance(child, tk.Frame) and child.winfo_children():
                            for grandchild in child.winfo_children():
                                if isinstance(grandchild, tk.Frame) and "time_frame" in str(grandchild):
                                    for time_label in grandchild.winfo_children():
                                        if isinstance(time_label, tk.Label) and time_label.cget("text").count(":") == 1:
    ...
    def show_home_page(self):
        ...
        current_time = tk.Label(time_frame, text="0:00", font=self.small_font, fg=self.text_color, bg="white")
        current_time.pack(side=tk.LEFT)
        
        total_time = tk.Label(time_frame, text="3:45", font=self.small_font, fg=self.text_color, bg="white")
        total_time.pack(side=tk.RIGHT)
